...
import { Search, Trash2, Loader2, Users, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function Students() {
  ...
  const toggleAll = () => {
    if (selected.size === pageEntries.length) {
      setSelected(new Set());
    } else {
      setSelected(new Set(pageEntries.map(e => e.id)));
    }
  };

  const handleDelete = async (ids: string[]) => {
    if (!confirm(`Delete ${ids.length} entries? / ${ids.length} एंट्री हटाएं?`)) return;
    const { error } = await supabase.from('student_entries').delete().in('id', ids);
    if (error) {
      toast.error('Failed to delete / हटाने में विफल');
    } else {
      setEntries(prev => prev.filter(e => !ids.includes(e.id)));
      setSelected(new Set());
      toast.success(`${ids.length} entries deleted / ${ids.length} एंट्री हटाई गई`);
    }
  };
  ...
  return (
    <DashboardLayout>
      <div className="flex items-center justify-between mb-6">
        ...
        <p className="text-sm text-muted-foreground">{filtered.length} entries / एंट्री</p>
      </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <Input
                ...
                placeholder="Search name, roll, mobile / नाम, रोल, मोबाइल खोजें"
                className="pl-10"
              />
            </div>
            <Select value={deptFilter} onValueChange={setDeptFilter}>
              ...
              <SelectContent>
                <SelectItem value="all">All Dept / सभी</SelectItem>
            ...
            <Select value={yearFilter} onValueChange={setYearFilter}>
              ...
              <SelectContent>
                <SelectItem value="all">All Year / सभी</SelectItem>
      ...
      <Card className="shadow-card">
        {loading ? (
          ...
          <CardContent className="p-10 text-center text-muted-foreground">
            No entries found / कोई एंट्री नहीं मिली
          </CardContent>
        ) : (
          <>
                ...
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-10">
                      <input type="checkbox" checked={selected.size === pageEntries.length && pageEntries.length > 0} onChange={toggleAll} className="rounded" />
                    </TableHead>
                    <TableHead>Name / नाम</TableHead>
                    <TableHead>Dept / विभाग</TableHead>
                    <TableHead>Year / वर्ष</TableHead>
                    <TableHead>Roll No</TableHead>
                    <TableHead>Mobile</TableHead>
                    <TableHead>Date / दिनांक</TableHead>
                    <TableHead>Time / समय</TableHead>
